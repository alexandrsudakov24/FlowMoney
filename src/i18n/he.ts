import type { TranslationKeys } from './en';

const he: Record<TranslationKeys, string> = {
    dashboard: 'לוח מחוונים',
    add_transaction: 'הוסף עסקה',
    profile: 'פרופיל',
    logout: 'התנתקות',
    login: 'כניסה',
    register: 'הרשמה',
    settings: 'הגדרות',
    theme: 'ערכת נושא',
    language: 'שפה',
    english: 'English',
    russian: 'Русский',
    hebrew: 'עברית',
    light: 'בהיר',
    dark: 'אפל',
    accent_color: 'צבע הדגשה',
    purple: 'סגול',
    blue: 'כחול',
    green: 'ירוק',
    red: 'אדום',
    orange: 'כתום',
    pink: 'ורוד',
    teal: 'טורקיז',
    indigo: 'אינדיגו',
    amber: 'ענבר',
    close: 'סגור',
    loading: 'טוען…',
    more_details: 'תאריך, חזרה, הערה',
    avatar: 'תמונת פרופיל',
    remove_filter: 'הסר מסנן',
    transaction_type: 'סוג עסקה',
    save: 'שמור',
    saving: 'שומר…',
    saved: 'נשמר',
    expense: 'הוצאה',
    income: 'הכנסה',
    total_income: 'סה"כ הכנסות',
    total_expenses: 'סה"כ הוצאות',
    net_balance: 'יתרה נקייה',
    net: 'נטו',
    today: 'היום',
    this_week: 'השבוע',
    this_month: 'החודש',
    expense_transactions: 'עסקאות הוצאה',
    hi: 'שלום',
    recent_transactions: 'עסקאות אחרונות',
    analytics: 'אנליטיקה',
    category: 'קטגוריה',
    amount: 'סכום',
    amount_placeholder: 'הכנס סכום',
    amount_required: 'הסכום חייב להיות גדול מאפס',
    date: 'תאריך',
    notes: 'הערה',
    note_placeholder: 'הוסף הערה (אופציונלי)',
    add: 'הוסף',
    edit: 'ערוך',
    delete: 'מחק',
    cancel: 'בטל',
    name: 'שם',
    email: 'דוא"ל',
    password: 'סיסמה',
    confirm_password: 'אשר סיסמה',
    select_language: 'בחר שפה',
    already_have_account: 'כבר יש לך חשבון? כניסה',
    create_account: 'צור חשבון',
    no_transactions: 'אין עסקאות עדיין',
    no_transactions_desc: 'התחל לעקוב על ידי הוספת עסקה חדשה',
    no_note: 'ללא הערה',
    delete_confirm: 'האם אתה בטוח שברצונך למחוק עסקה זו?',
    cat_food: 'אוכל',
    cat_transport: 'תחבורה',
    cat_home: 'בית',
    cat_shopping: 'קניות',
    cat_health: 'בריאות',
    cat_other: 'אחר',
    cat_salary: 'משכורת',
    cat_freelance: 'פרילנס',
    cat_dividends: 'דיבידנדים',
    cat_gift: 'מתנה',
    edit_transaction: 'ערוך עסקה',
    expense_not_found: 'העסקה לא נמצאה.',
    go_back: 'חזרה',
    or: 'או',
    login_with_google: 'כניסה עם Google',
    register_with_google: 'הרשמה עם Google',
    stats: 'סטטיסטיקה',
    transactions: 'עסקאות',
    top_category: 'קטגוריה מובילה',
    data_export: 'ייצוא נתונים',
    export_json: 'ייצוא ל-JSON',
    change: 'שנה',
    currency: 'מטבע',
    welcome_title: 'ברוכים הבאים ל-FlowMoney',
    welcome_lead: 'עקוב אחר הוצאותיך, הצג את ההוצאות ועמוד ביעדים.',
    go_to_dashboard: 'עבור ללוח המחוונים',
    chart_by_category: 'לפי קטגוריה',
    no_expense_data: 'אין נתוני הוצאות',
    current_currency: 'מטבע נוכחי',
    current_theme: 'ערכת נושא נוכחית',
    toggle_theme: 'החלף ערכת נושא',
    categories: 'קטגוריות',
    add_category: 'שם קטגוריה חדשה',
    category_exists: 'הקטגוריה כבר קיימת',
    category_in_use: 'הקטגוריה בשימוש בעסקאות',
    category_default: 'לא ניתן למחוק קטגוריה ברירת מחדל',
    monthly_limit: 'תקציב חודשי',
    no_limit: 'ללא הגבלה',
    set_limit: 'הגדר תקציב',
    remove_limit: 'הסר תקציב',
    continue: 'המשך',
    confirm: 'כן, הגדר',
    confirm_limit_title: 'להגדיר תקציב חודשי?',
    budgets: 'תקציבים',
    budget_exceeded: 'התקציב חורג',
    filter_from: 'מ-',
    filter_until: 'עד',
    smart_search_placeholder: 'שאל בשפה חופשית… למשל "מתנות השנה"',
    smart_search_button: 'חיפוש AI',
    smart_search_error: 'לא הצלחנו להבין את החיפוש',
    smart_search_no_results: 'לא נמצאו תוצאות',
    receipt_scan: 'קבלה מצילום',
    receipt_scan_hint: 'צילום ← ה-AI ימלא',
    draft_will_add_expense: 'ההוצאה הבאה תתווסף:',
    draft_will_add_income: 'ההכנסה הבאה תתווסף:',
    draft_add: 'הוסף',
    draft_edit: 'ערוך',
    voice_input: 'בקול',
    voice_input_hint: '"קפה 15 אתמול"',
    voice_type_hint: 'כתבו משפט — ה-AI ימלא',
    voice_listening: 'מקשיב…',
    voice_parsing: 'מפענח…',
    voice_placeholder: 'למשל: "קניות 230 אתמול"',
    voice_submit: 'אישור',
    voice_stop: 'עצור',
    voice_not_recognized: 'לא הצלחנו להבין את הסכום. נסו לנסח אחרת.',
    voice_no_speech: 'לא נשמע דבר. נסו שוב.',
    voice_error_mic: 'אין גישה למיקרופון. אפשרו אותה בהגדרות הדפדפן.',
    receipt_scanning: 'קורא את הקבלה…',
    receipt_not_recognized: 'לא נמצא סכום קבלה בתמונה. נסו צילום ברור יותר.',
    ai_input_error_rate_limited: 'יותר מדי בקשות. נסו שוב בעוד רגע.',
    ai_input_error_blocked: 'ה-AI לא הצליח לעבד את זה. נסו שוב.',
    ai_input_error_parse_error: 'משהו השתבש בזיהוי. נסו שוב.',
    ai_input_error_network_error: 'שירות ה-AI אינו זמין כרגע. נסו שוב מאוחר יותר.',
    ai_input_error_no_api_key: 'קלט AI אינו מוגדר.',
    ai_input_error_invalid_api_key: 'קלט AI אינו זמין כי מפתח ה-API שגוי או לא מופעל.',
    budget_tips: 'טיפים שבועיים לתקציב',
    budget_tips_generate_cta: 'קבל טיפים',
    budget_tips_refresh_cta: 'רענן טיפים',
    budget_tips_cooldown_hint: 'טיפים חדשים יהיו זמינים בעוד {time}',
    budget_tips_empty: 'קבל עצה מבינה מלאכותית איך לעמוד בתקציב הקטגוריות השבוע.',
    budget_tips_error_rate_limited: 'יותר מדי בקשות. נסה שוב בעוד רגע.',
    budget_tips_error_blocked: 'הבינה המלאכותית לא הצליחה ליצור טיפים עבור הנתונים האלה. נסה שוב מאוחר יותר.',
    budget_tips_error_parse_error: 'משהו השתבש ביצירת הטיפים. נסה שוב.',
    budget_tips_error_network_error: 'שירות ה-AI אינו זמין כרגע. נסה שוב מאוחר יותר.',
    budget_tips_error_no_api_key: 'טיפי תקציב מבוססי AI אינם מוגדרים.',
    budget_tips_error_invalid_api_key: 'טיפי תקציב מבוססי AI אינם זמינים בגלל מפתח API שגוי או לא מופעל.',
    money_chat_title: 'שאל על הכסף שלך',
    money_chat_placeholder: 'למשל, "כמה הוצאתי על מוניות ביולי?"',
    money_chat_ask: 'שאל',
    money_chat_error_rate_limited: 'יותר מדי בקשות. נסה שוב בעוד רגע.',
    money_chat_error_blocked: 'הבינה המלאכותית לא הצליחה לענות. נסה לנסח מחדש.',
    money_chat_error_parse_error: 'משהו השתבש. נסה שוב.',
    money_chat_error_network_error: 'שירות ה-AI אינו זמין כרגע. נסה שוב מאוחר יותר.',
    money_chat_error_no_api_key: 'התכונה הזו אינה מוגדרת.',
    money_chat_error_invalid_api_key: 'התכונה אינה זמינה בגלל מפתח API שגוי או לא מופעל.',
    app_version: 'גרסת אפליקציה',
    share_app: 'שתף את האפליקציה',
    share_app_text: 'FlowMoney — מעקב פיננסי אישי. נסו את זה!',
    share_link_copied: 'הקישור הועתק',
    about_app: 'אודות האפליקציה',
    about_app_description: 'FlowMoney היא אפליקציית מעקב פיננסי אישי מסוג PWA. עקבו אחרי ההכנסות וההוצאות שלכם, נתחו הוצאות בעזרת גרפים, שתפו תקציב עם המשפחה וקבלו תובנות מבוססות בינה מלאכותית על ההוצאות שלכם.',
    auto: 'אוטומטי',
    family: 'משפחה',
    family_budget: 'תקציב משפחתי',
    create_family: 'צור משפחה',
    family_name: 'שם המשפחה',
    family_name_placeholder: 'לדוגמה: משפחת כהן',
    invite_member: 'הזמן חבר',
    invite_email_placeholder: 'כתובת דוא"ל',
    invite: 'הזמן',
    leave_family: 'עזוב משפחה',
    leave_family_confirm: 'האם אתה בטוח שברצונך לעזוב את המשפחה?',
    remove_member: 'הסר',
    remove_member_confirm: 'להסיר את {name} מהמשפחה?',
    members: 'חברים',
    owner: 'בעלים',
    pending_invitations: 'הזמנות ממתינות',
    accept: 'קבל',
    decline: 'דחה',
    invited_by: 'הוזמן על ידי',
    invite_success: 'ההזמנה נשלחה!',
    invite_error_already_member: 'אדם זה כבר חבר',
    invite_error_already_invited: 'ההזמנה כבר נשלחה',
    save_error: 'שמירה נכשלה. אנא נסה שוב.',
    added_by: 'על ידי',
    no_family: 'אינך במשפחה עדיין',
    no_family_desc: 'צור משפחה לניהול תקציב משותף',
    no_invitations: 'אין הזמנות ממתינות',
    admin: 'מנהל',
    admin_panel: 'לוח ניהול',
    all_users: 'כל המשתמשים',
    delete_user: 'מחק',
    delete_user_confirm: 'למחוק משתמש זה וכל הנתונים שלו? לא ניתן לבטל.',
    no_users: 'לא נמצאו משתמשים',
    user_deleted: 'המשתמש נמחק',
    contact_developer: 'צור קשר עם המפתח',
    feedback_placeholder: 'מה כדאי לשפר? ספר לנו מה בראש שלך…',
    feedback_sent: 'ההודעה נשלחה, תודה!',
    feedback: 'משוב',
    no_feedback: 'אין הודעות עדיין',
    send: 'שלח',
    delete_feedback_confirm: 'למחוק את ההודעה? לא ניתן לבטל פעולה זו.',
    registered: 'נרשם',
    filter_all_time: 'כל הזמן',
    filter_search_placeholder: 'חפש לפי הערה או קטגוריה...',
    filter_type_all: 'הכל',
    filter_no_results: 'אין עסקאות התואמות לפילטרים שלך',
    clear_filters: 'נקה מסננים',
    filter_results_count: 'מציג {n} מתוך {total}',
    load_more: 'הצג עוד',
    not_found: 'הדף לא נמצא',
    continue_as_guest: 'המשך ללא חשבון',
    select_currency: 'מטבע',
    error_email_in_use: 'כתובת האימייל כבר רשומה. התחבר כדי לשמור את נתוני האורח.',
    error_invalid_credential: 'אימייל או סיסמה שגויים.',
    error_invalid_email: 'כתובת אימייל לא חוקית.',
    error_weak_password: 'הסיסמה חייבת להכיל לפחות 8 תווים, אות גדולה וספרה, ורק אותיות וספרות (ללא רווחים או סימנים).',
    error_too_many_requests: 'יותר מדי ניסיונות. נסה שוב מאוחר יותר.',
    error_network: 'שגיאת רשת. בדוק את החיבור.',
    error_popup_closed: 'חלון ההתחברות נסגר. נסה שוב.',
    error_unknown: 'משהו השתבש. נסה שוב.',
    error_boundary_title: 'משהו השתבש',
    error_boundary_message: 'אירעה שגיאה בלתי צפויה. נסה לטעון מחדש את הדף.',
    reload_page: 'טען מחדש',
    select_theme: 'ערכת נושא',
    save_your_data: 'שמור את הנתונים שלך',
    save_your_data_hint: 'צור חשבון או התחבר — העסקאות שלך ישמרו.',
    schedule_later_hint: 'ייושם אוטומטית כשיגיע התאריך',
    schedule_date_future_required: 'בחר תאריך שמאוחר מהיום',
    scheduled_payments: 'תשלומים מתוזמנים',
    no_scheduled_payments: 'אין תשלומים מתוזמנים',
    repeat: 'חזרה',
    repeat_none: 'עכשיו',
    repeat_once: 'פעם אחת',
    repeat_monthly: 'כל חודש',
    repeat_monthly_hint: 'עסקה חדשה תתווסף אוטומטית כל חודש בתאריך הזה',
    category_from_history: 'נבחר לפי עסקאות קודמות',
    insights_error_rate_limited: 'יותר מדי בקשות. נסה שוב בעוד רגע.',
    insights_error_blocked: 'ה-AI לא הצליח ליצור תובנות עבור נתונים אלה. נסה שוב מאוחר יותר.',
    insights_error_parse_error: 'משהו השתבש ביצירת התובנות. נסה שוב.',
    insights_error_network_error: 'שירות ה-AI אינו זמין זמנית. נסה שוב מאוחר יותר.',
    insights_error_no_api_key: 'תובנות AI אינן מוגדרות.',
    insights_error_invalid_api_key: 'תובנות AI אינן זמינות: מפתח ה-API אינו תקף או לא פעיל.',
    insights: 'ניתוח הוצאות',
    insights_teaser_desc: 'הצצה מהירה על מה שאתה מוציא עליו.',
    insights_empty: 'עדיין אין תובנות. צור את הראשונה כדי לראות ניתוח של ההוצאות שלך.',
    insights_generate_cta: 'צור תובנות',
    insights_regenerate_cta: 'רענן',
    insights_cooldown_hint: 'הרענון הבא יהיה זמין בעוד {time}',
    insights_last_updated: 'עודכן {time}',
    rollover_settings: 'העברת יתרה',
    carried_over: 'הועבר מהחודש שעבר: {amount}',
    rollover_mode: 'כאשר מתחיל חודש חדש',
    rollover_reset: 'להתחיל מחדש',
    rollover_reset_hint: 'העודף או החריגה לא מועברים — כל חודש מתחיל מאפס.',
    rollover_full: 'להעביר הכול',
    rollover_full_hint: 'העודף מתווסף לחודש הבא; החריגה מנוכה ממנו.',
    rollover_surplus_only: 'להעביר רק עודף',
    rollover_surplus_only_hint: 'כסף שלא נוצל מועבר הלאה, אך חריגה לא מנוכה.',
    rollover_deficit_only: 'להעביר רק חריגה',
    rollover_deficit_only_hint: 'חריגה מנוכה מהחודש הבא, אך עודף לא מועבר.',
};

export default he;
